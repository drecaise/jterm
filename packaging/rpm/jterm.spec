# jpackage resource override for the RPM spec (picked up via --resource-dir in
# .github/workflows/release.yml). Based on JDK 21's jdk.jpackage template.spec;
# ALL_CAPS tokens are substituted by jpackage.
#
# Diff from the stock template: the desktop entry is moved out of the app image
# and packaged as /usr/share/applications/jterm.desktop (a real, RPM-tracked
# file), replacing the stock %post/%preun xdg-desktop-menu scriptlets that
# copied an untracked jterm-jterm.desktop into /usr/local/share/applications.
...
# Relocate the jpackage-generated desktop entry (lib/jterm-jterm.desktop) to
# the standard XDG location under its canonical name. Picked up by the
# auto-generated file list below, so RPM owns and removes it.
install -d -m 755 %{buildroot}%{_datadir}/applications
mv %{buildroot}APPLICATION_DIRECTORY/lib/*.desktop \
